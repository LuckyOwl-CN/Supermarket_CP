    ...
    while (select != 10) {
        ...
        switch (select) {
            ...
            case 3://排序
                ...
                break;
            case 4:
                findOrderInformation(loginUser,head);
                break;
            default:
                getchar();
                rewind(stdin);//清除键盘缓存
...
void showShop() {
	...
	if (head == NULL) {
		...
		while (p != NULL) {
			printf("%5s\t%5s\t%5s\t%5s\t%5s\t%d\t%5s\t%5s\n", p->ID, p->type, p->number, p->date, p->day, p->amount, p->buyer, p->saletime);
...
void readInformation() {
	...
	pf = (SHOP*)malloc(sizeof(SHOP));

	int i = 0;
	head = (SHOP*)malloc(sizeof(SHOP));
	head->next = NULL;
	if ((fp = fopen("CommodityInf.txt", "r")) == NULL) {
		printf("文件不存在。\n");
		return;
	}
	printf("文件存在。\n");
...
void findOrderInformation(userInf user,SHOP*head) {
